...
Controller::Controller(int channel, const Config &config, const Timing &timing)
    ...
    : channel_id_(channel),
      ...
      row_buf_policy_((config.row_buf_policy == "CLOSE_PAGE")
                          ...
                          : ((config.row_buf_policy == "OPEN_PAGE")
...
/* I guess this is of importance to us... how the scheduling is being done...*/
...
/* Place where the open close policy is implemented */
Command Controller::TransToCommand(const Transaction &trans) {
    ...
    } else if (row_buf_policy_== RowBufPolicy::OPEN_4){
