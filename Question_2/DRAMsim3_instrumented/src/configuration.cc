...
Address Config::AddressMapping(uint64_t hex_addr) const {
    ...
    //if cache interleaving is not intended then CO_mask shall be zero and hence CO shall be 0
    int CO = (hex_addr >> CO_pos) & CO_mask; 
    int bankBits=LogBase2(banks_per_group);
    //if cache interleaving is not intendedn then CO shall be zero and hence column shall be == co
    int column= (CO << (bankBits+3)) +co;
   return Address(channel, rank, bg, ba, ro, column);
}
...
void Config::InitSystemParams() {
    ...
    scheduling_policy = reader.Get("system", "scheduling_policy", "FCFS");
...
void Config::SetAddressMapping() {
    ...
    {
        //cache interleaved mode
        field_widths["co"]=3;
        field_widths["CO"]=actual_col_bits-3;
    ...
    if(field_pos.find("CO")!=field_pos.end())
        CO_pos = field_pos.at("CO");
    else
        CO_pos=0;
    ...
    //If the mode is not cache interleaved then field_width["CO"] shall have 0
    // and CO_mask shall have value 0
    CO_mask = (1 << field_widths.at("CO")) - 1; 
